# MVP Plan: IntelliView AI

## 1. Phase 1: Core Engine (Weeks 1-4)
*   **Week 1:** Setup Infrastructure (Docker, PostgreSQL, Redis).
*   **Week 2:** Implement CV Parsing (Python).
*   **Week 3:** Basic LLM Integration (Mistral-7B) via FastAPI.
*   **Week 4:** WebSocket Chat Implementation.

## 2. Phase 2: Recruiter Dashboard (Weeks 5-8)
*   **Week 5:** Auth & Tenant Management.
*   **Week 6:** Interview Link Generation & Expiry Logic.
*   **Week 7:** Report Generation Pipeline (Python + LaTeX/PDF).
*   **Week 8:** Dashboard UI (List views, status tracking).

## 3. Phase 3: Refinement & Launch (Weeks 9-12)
*   **Week 9:** Prompt Engineering & Evaluation Rubric tuning.
*   **Week 10:** Load Testing (Goal: 100 concurrent interviews).
*   **Week 11:** Beta testing with 3 partner companies.
*   **Week 12:** V1.0 Release.

## 4. Milestones
1.  **M1 (Week 4):** First successful AI-led chat session using a real CV.
2.  **M2 (Week 8):** Recruiter can view a generated report in the dashboard.
3.  **M3 (Week 12):** Public beta launch.

## 5. Go-To-Market (GTM)
*   **Free Pilot:** Offer 10 free interviews to YC-style startups.
*   **LinkedIn Outreach:** Target Engineering Managers directly.
*   **ProductHunt Launch:** Focus on the "No more LeetCode" angle for candidates.
